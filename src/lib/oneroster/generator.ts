    ...
    for (const courseId of courseIds) {
      ...
      for (const termId of terms) {
        ...
        classes.push({
          ...
          classCode: `${course.courseCode}-${grade}`,
          classType: "scheduled",
  ...
  for (const schoolId of schoolIds) {
    ...
    for (const courseId of courseIds) {
      ...
      users.push({
        ...
        role: "teacher",
        username: `${givenName.toLowerCase()}.${familyName.toLowerCase()}@school.edu`,
        userIds: "",
        givenName,
        familyName,
        middleName: "",
        identifier: faker.string.alphanumeric(8).toUpperCase(),
        email: `${givenName.toLowerCase()}.${familyName.toLowerCase()}@school.edu`,
        sms: "",
  ...
  for (const schoolId of schoolIds) {
    ...
    for (let i = 0; i < studentsPerSchool; i++) {
      ...
      users.push({
        ...
        orgSourcedIds: schoolId,
        role: "student",
        username: `${givenName.toLowerCase()}${familyName.toLowerCase()}${faker.string.numeric(4)}`,
        userIds: "",
        givenName,
        familyName,
        middleName: faker.helpers.maybe(() => faker.person.middleName(), { probability: 0.3 }) ?? "",
        identifier: faker.string.alphanumeric(8).toUpperCase(),
        email: `${givenName.toLowerCase()}.${familyName.toLowerCase()}@students.school.edu`,
        sms: "",
        phone: "",
        agentSourcedIds: "",
        grades: grade,
      ...
      if (includeDemographics) {
        ...
        demographics.push({
          ...
          status: "active",
          dateLastModified: NOW,
          birthDate: isoDate(new Date(`${birthYear}-${faker.string.numeric({ length: 2, allowLeadingZeros: true })}-01`)),
